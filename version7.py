...
# Class IDs for vehicles in COCO dataset and their categories
vehicle_classes = {
    2: "Car",         # Car
...
while cap.isOpened():
    ...
    # Filter results to keep only vehicle classes and extract bounding box coordinates
    for det in detections:
        x1, y1, x2, y2, conf, class_id = map(int, det[:6])
        if class_id in vehicle_classes:
            vehicle_boxes.append([x1, y1, x2, y2])  # Only pass coordinates, excluding conf and class_id
    ...
    for bbox in tracked_boxes:
        x3, y3, x4, y4, vehicle_id = bbox  # Only unpack five values here
        ...
        # Get the vehicle category based on class_id
        vehicle_category = vehicle_classes.get(class_id, "Unknown")
        ...
        if vehicle_id in entry_tracking:
            if line_start_red[1] < (cy + offset) and line_start_red[1] > (cy - offset):
                ...
                cv2.putText(frame, f'{vehicle_id} ({vehicle_category})', (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
