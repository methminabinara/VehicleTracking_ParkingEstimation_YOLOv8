...
while cap.isOpened():
    ...
    for det in detections:
        ...
        if int(class_id) in vehicle_classes:
            ...
            vehicle_boxes.append((x, y, w, h, int(class_id)))

    # Update tracker and get tracked boxes with IDs
    tracked_boxes = tracker.update([(x, y, w, h) for x, y, w, h, _ in vehicle_boxes])

    # Draw bounding boxes and display vehicle types
    for (x, y, w, h, vehicle_id), (_, _, _, _, class_id) in zip(tracked_boxes, vehicle_boxes):
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
        cv2.putText(frame, f'Class:{class_id} ID:{vehicle_id}', (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
    ...
    # Display entry, exit counts on the frame
